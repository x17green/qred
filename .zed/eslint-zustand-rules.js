  ...
  rules: {
    ...
    "zustand-object-selector-shallow": {
      meta: {
        ...
        docs: {
          description:
            "Require useShallow for object destructuring in Zustand selectors",
        ...
        messages: {
          missingUseShallow:
            "Object destructuring in useStore selector requires useShallow to prevent infinite loops in Zustand v5+",
          addUseShallowImport:
            'Add useShallow import: import { useShallow } from "zustand/react/shallow"',
        },
      },
      create(context) {
        return {
          CallExpression(node) {
            ...
            if (
              ...
              if (
                ...
                const hasUseShallow = node.arguments.some(
                  (arg) =>
    ...
    "zustand-no-array-methods-in-selector": {
      meta: {
        ...
        docs: {
          description:
            "Prevent array methods in Zustand selectors that create new references",
          category: "Possible Errors",
          recommended: true,
        },
        messages: {
          noArrayMethods:
            "Array methods ({{method}}) in selectors create new references. Move to useMemo in component.",
        },
      },
      create(context) {
        const arrayMethods = [
    ...
    "zustand-no-computations-in-selector": {
      meta: {
        ...
        docs: {
          description:
            "Prevent computations in Zustand selectors that could cause performance issues",
          category: "Best Practices",
          recommended: true,
        },
        messages: {
          noComputations:
            "Avoid computations in selectors. Consider moving to useMemo or computed hook.",
        },
      },
      create(context) {
        return {
                ...
                selector.body.properties.forEach((prop) => {
                  if (
                    ...
                    (prop.value.type === "MemberExpression" &&
                      ["reduce", "length", "map", "filter"].includes(
    ...
    "zustand-require-use-shallow-import": {
      meta: {
        ...
        docs: {
          description:
            "Require useShallow import when using object destructuring patterns",
          category: "Possible Errors",
          recommended: true,
        },
        fixable: "code",
        messages: {
          missingImport:
            'Missing useShallow import. Add: import { useShallow } from "zustand/react/shallow"',
        },
      },
      create(context) {
        ...
        return {
          ImportDeclaration(node) {
            if (node.source.value === "zustand/react/shallow") {
              const hasUseShallowSpecifier = node.specifiers.some(
                (spec) =>
                  spec.type === "ImportSpecifier" &&
                  spec.imported.name === "useShallow",
              );
