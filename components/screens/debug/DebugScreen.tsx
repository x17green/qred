          ...
          <Box className="bg-success-50 p-4 rounded-lg border border-success-200">
            ...
            <VStack space="sm">
              <Text size="sm" className="text-success-700">
                <Text className="font-semibold">BASE_URL:</Text> {API_CONFIG.BASE_URL}
              </Text>
              <Text size="sm" className="text-success-700">
                <Text className="font-semibold">From .env:</Text> {process.env.EXPO_PUBLIC_API_URL || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-success-700">
                <Text className="font-semibold">Match:</Text> {
                  API_CONFIG.BASE_URL === (process.env.EXPO_PUBLIC_API_URL || 'http://localhost:3000/api/v1') ? '✅ YES' : '❌ NO'
                }
              </Text>
            </VStack>
          </Box>
          ...
          <Box className="bg-error-50 p-4 rounded-lg border border-error-200">
            ...
            <VStack space="sm">
              <Text size="sm" className="text-error-700">
                <Text className="font-semibold">Paystack Test:</Text> {PAYMENT_CONFIG.PAYSTACK.TEST_PUBLIC_KEY || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-error-700">
                <Text className="font-semibold">Paystack Live:</Text> {PAYMENT_CONFIG.PAYSTACK.LIVE_PUBLIC_KEY || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-error-700">
                <Text className="font-semibold">Flutterwave Test:</Text> {PAYMENT_CONFIG.FLUTTERWAVE.TEST_PUBLIC_KEY || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-error-700">
                <Text className="font-semibold">Flutterwave Live:</Text> {PAYMENT_CONFIG.FLUTTERWAVE.LIVE_PUBLIC_KEY || 'NOT SET'}
              </Text>
            </VStack>
          </Box>
          ...
          <Box className="bg-background-100 p-4 rounded-lg border border-background-300">
            ...
            <VStack space="sm">
              <Text size="sm" className="text-typography-600">
                <Text className="font-semibold">App Name:</Text> {process.env.EXPO_PUBLIC_APP_NAME || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-typography-600">
                <Text className="font-semibold">App Version:</Text> {process.env.EXPO_PUBLIC_APP_VERSION || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-typography-600">
                <Text className="font-semibold">Environment:</Text> {process.env.EXPO_PUBLIC_APP_ENV || 'NOT SET'}
              </Text>
              <Text size="sm" className="text-typography-600">
                <Text className="font-semibold">Dev Mode:</Text> {process.env.EXPO_PUBLIC_DEV_MODE || 'NOT SET'}
              </Text>
            </VStack>
          </Box>
          ...
          <Box className="bg-info-50 p-4 rounded-lg border border-info-200">
            ...
            <VStack space="sm">
              <Text size="sm" className="text-info-700">
                • All EXPO_PUBLIC_* variables should be loaded from your .env file
              </Text>
              <Text size="sm" className="text-info-700">
